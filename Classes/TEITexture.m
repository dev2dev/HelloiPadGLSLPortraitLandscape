...
- (id)initWithTextureFile:(NSString *)name mipmap:(BOOL)mipmap {
	...
	if(nil != self) {
		...
		if (ui_image.CGImage != NULL) {
			...
			_width	= NextPowerOfTwo(CGImageGetWidth(ui_image.CGImage));
			_height	= NextPowerOfTwo(CGImageGetHeight(ui_image.CGImage));
...
- (BOOL)ingestPVRTextureFile:(NSData *)data {
	...
	uint32_t w = 0;
	uint32_t h = 0;
	_width	= w = CFSwapInt32LittleToHost(header->width);
	_height	= h = CFSwapInt32LittleToHost(header->height);
	...
	while (dataOffset < dataLength) {
		...
		[ _pvrTextureData addObject:[ NSData dataWithBytes:my_bytes + dataOffset length:dataSize ] ];
...
//			  GL_REPLACE);	
	...
	w = _width;
	h = _height;
	for (int i = 0; i < [_pvrTextureData count]; i++) {
		
		NSData *data = [_pvrTextureData objectAtIndex:i];
